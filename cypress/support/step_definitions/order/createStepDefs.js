...
Given('Click on the Create Order button', () => {
  cy.get('[data-test="btn-create-order"]').click();
  cy.wait(2000);
});

When('Input customer data on address auto spread: {string}', (customer) => {
...
Then('Choose payment for this order: {string}', (paymentMethod) => {
...
Then('Confirm payment for bank tranfers', () => {
  cy.get('[data-test="checkbox-confirm-payment"]').click();
  cy.wait(1500);
});

Then('Close toggle for bank tranfers', () => {
  cy.get('.v-switch-core').click();
  cy.wait(1500);
});

Then('Mark confirm order for COD', () => {
  cy.get('[data-test="checkbox-confirm-cod"]').click();
  cy.wait(1500);
});

Then('Add product in order', () => {
...
Then('Click Save button for create order', () => {
